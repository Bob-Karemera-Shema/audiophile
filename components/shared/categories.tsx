...
const Categories = ({ variant = 'section' }: { variant: 'section' | 'sidebar' }) => {
    return (
        <section
            className={clsx(
                'grid grid-cols-1 sm:grid-cols-3 gap-y-16 gap-x-2 lg:gap-x-8',
                {
                    'pt-18 pb-8': variant === 'sidebar',
                    'mt-32 mb-16 py-14': variant === 'section'
                }
            )}
        >
            {
                    ...
                    return (
                        <Link key={name + url} href={url} aria-label={`${name} Page`}>
                            <div className="bg-container-gray rounded-md flex flex-col items-center justify-end group pb-4 h-40">
                                ...
                                <h2 className="relative -top-2 font-bold text-[15px] text-black uppercase tracking-wider mb-4">{name}</h2>
